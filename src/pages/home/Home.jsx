      ...
      <>
        <section class="bg-default-white dark:bg-dark-one bg-center bg-no-repeat bg-cover h-screen w-full overflow-hidden">
          <main class="main flex justify-center items-center flex-col px-:2 bg-none h-medium w-11/12 center">
            ...
            {options.map(option => 
              <router-link to={option.to} class="flex justify-center font-ralewayMedium bg-dark-one dark:bg-white-one border-none py-:2 my-:1 w-8/12 text-lg cursor-pointer no-underline shadow-2xl transition-all duration-200 dark:text-default-black dark:hover:bg-default-blueLight hover:bg-default-blueLight hover:text-dark-one dark:hover:text-default-black ">{option.title}</router-link>
            )}
          </main>
        </section>
      </>
